...
function render_sankey(allNodes, allFlows, cfg, numberStyle) {
  ...
  function getLabelPieces(n, magnification) {
    const overallSize = cfg.labelname_size * magnification,
        ...
        parts = String(n.name).split(/\\b/),
        nameObjs = [];
    
    // Handle main label (everything before \b)
    const mainLabel = parts[0].split(/\\n/);
    mainLabel.forEach((part, i) => {
        nameObjs.push({
            txt: part,
            weight: cfg.labelname_weight,
            size: i === 0 ? nameSize : nameSize * 0.8,
            color: i === 0 ? cfg.labels_color : '#808080',
            newLine: i > 0,
            isBottom: false
        });
    });

    // Handle bottom text if present
    if (parts[1]) {
        n.bottomText = {
            txt: parts[1],
            weight: cfg.labelname_weight,
    ...
    switch (cfg.labelvalue_position) {
        ...
        case 'after':
            nameObjs[nameObjs.length - 1].txt += ': ';
        default:
            return [...nameObjs, valObj];
    }
  }
  /**
  ...
  if (!cfg.labels_hide && (cfg.labelname_appears || cfg.labelvalue_appears)) {
    ...
    diagLabels.selectAll()
      ...
      .append('text')
        ...
        .attr('fill', (n) => n.labelList[0].color)
  ...
  diagLabels.selectAll()
    .data(allNodes.filter(n => n.labelList.some(l => l.isBottom)))
    .enter()
    .append('text')
      .attr('id', n => `${n.dom_id}_bottom`)
      .attr('class', n => n.css_class)
      .attr('text-anchor', 'middle')
      .attr('x', n => ep(n.x + n.dx/2))  // Center horizontally under the node
      .attr('y', n => ep(n.y + n.dy + cfg.labelname_size * 0.8))  // Position below node
      .attr('font-weight', n => n.labelList.find(l => l.isBottom).weight)
      .attr('font-size', n => `${ep(n.labelList.find(l => l.isBottom).size)}px`)
      .attr('fill', n => n.labelList.find(l => l.isBottom).color)
      .text(n => n.labelList.find(l => l.isBottom).txt);

      const diagBottomLabels = diagMain.append('g')
      .attr('id', 'sankey_bottom_labels')
      .attr('font-family', cfg.labels_fontface)
      .attr('font-size', `${ep(cfg.labelname_size)}px`)
      .attr('fill', cfg.labels_color);

    // Add bottom text labels
    diagBottomLabels.selectAll()
      .data(allNodes.filter(n => n.bottomText))
      .enter()
      .append('text')
        .attr('id', n => `${n.dom_id}_bottom`)
        .attr('class', n => n.css_class)
        .attr('text-anchor', 'middle')
        .attr('x', n => ep(n.x + n.dx/2))  // Center horizontally under the node
        .attr('y', n => ep(n.y + n.dy + cfg.labelname_size * 1.2))  // Position below node
        .attr('font-weight', n => n.bottomText.weight)
        .attr('font-size', n => `${ep(n.bottomText.size)}px`)
        .attr('fill', n => n.bottomText.color)
        .text(n => n.bottomText.txt);

          function addNodeLabels(diagLabels, nodes) {
            diagLabels.selectAll()
              .data(nodes.filter(n => !n.hideLabel))
              .enter()
              .append('text')
              .attr('id', n => n.label.dom_id)
              .attr('class', n => n.css_class)
              ...
              .each(function(n) {
                ...
                // Add top text
                text.append('tspan')
                  .attr('dy', ep(n.label.dy))
                  .text(n.labelList[0].txt);
                  
                // Add bottom text if present
                if (n.bottomText) {
                  text.append('tspan')
                    .attr('x', ep(n.label.x))
                    .attr('dy', ep(n.dy + n.height - 2)) // Position at bottom
                    .text(n.bottomText);
                }
              });
          }
...
glob.process_sankey = () => {
   ...
   */
// In sankeymatic.js, modify the parseNodeName function to handle \b:
  function parseNodeName(nodeName) {
    const hideLabel = nodeName.startsWith('~');
    const trueName = hideLabel ? nodeName.slice(1) : nodeName;
    const hasBottomText = trueName.includes('\\b');
    const [topText, bottomText] = hasBottomText ? trueName.split('\\b') : [trueName, ''];
    
    return {
      trueName: topText + (bottomText ? '\n' + bottomText : ''),
      hideLabel,
